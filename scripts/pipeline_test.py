...
pipe = pipeline(model="microsoft/beit-large-finetuned-ade-640-640", task="image-segmentation")
res = pipe(file)
# res[0]['mask'].show()
...
for mask in res:
    ...
    for label, color in enumerate(palette):
        color_seg[pred_seg != 0, :] = palette[label2id[mask['label']]]  # color the segmentation map
