...
def analyze_segmentation_map(seg_map: np.ndarray, num_categories: int) -> Dict[int, Tuple[int, float]]:
...
def initialize_csv_files(output_prefix: Path, category_names: Dict[int, str]):
    ...
    percentages_file = output_prefix.with_name(f"{output_prefix.stem}_category_percentages.csv")
...
def append_to_csv_files(counts_file: Path, percentages_file: Path, frame_count: int, analysis: Dict[int, Tuple[int, float]]):
    ...
    pd.DataFrame([percentages_row]).to_csv(percentages_file, mode="a", header=False, index=False)
...
def save_segmentation_map(seg_map: np.ndarray, output_prefix: Path, frame_count: int = None):
...
def save_colored_segmentation(colored_seg: np.ndarray, output_prefix: Path, frame_count: int = None):
    filename = f"{output_prefix.stem}_colored{'_' + str(frame_count) if frame_count is not None else ''}.png"
    cv2.imwrite(str(output_prefix.with_name(filename)), cv2.cvtColor(colored_seg, cv2.COLOR_RGB2BGR))
