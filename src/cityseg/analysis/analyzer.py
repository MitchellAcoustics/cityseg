...
from ..utils.common import get_segmentation_batch
from ..storage.storage import ParquetAnalysisStorage
...
class SegmentationAnalyzer:
    ...
    @staticmethod
    def analyze_segmentation_dataset(dataset: xr.Dataset, output_path: Path) -> Path:
        """
    ...
    def analyze_results_legacy(
        ...
        headers = ["Frame"] + [
            id2label.get(str(i), f"category_{i}")
            ...
            for chunk_start in range(0, total_length, chunk_size):
                ...
                for frame_idx, seg_map in enumerate(seg_chunk, start=chunk_start):
                    ...
                    counts_row = [frame_number] + [
                        analysis.get(i, (0, 0.0))[0]
                        for i in sorted(range(len(id2label)))
                    ]
                    percentages_row = [frame_number] + [
                        analysis.get(i, (0, 0.0))[1]
                        for i in sorted(range(len(id2label)))
                    ]
