...
class SegmentationDataset:
    ...
    Data Variables:
        - segmentation: Integer labels for each pixel (dims: [time], y, x)
    ...
    REQUIRED_DATA_VARS = {"segmentation"}
    OPTIONAL_DATA_VARS = {"confidence"}
    ...
    def validate_dataset(cls, ds: xr.Dataset) -> None:
        ...
        # Validate segmentation data variable
        seg_var = ds.segmentation
        expected_dims = {"y", "x"}
        if "time" in ds.dims:
        ...
        if set(seg_var.dims) != expected_dims:
            raise ValueError(
                f"Segmentation variable has dims {seg_var.dims}, "
                f"expected {expected_dims}"
            )

        # Validate data types
        if not np.issubdtype(seg_var.dtype, np.integer):
            raise ValueError("Segmentation data must be integer type")
...
def create_segmentation_dataset(
    ...
    data_vars = {"segmentation": (dims, segmentation_data.astype(np.int32))}
