...
It encapsulates methods for loading and preprocessing images for segmentation.
"""
...
class ImageProcessor:
    ...
    def load_image(image_path: Path) -> Image.Image:
        ...
        try:
            ...
            logger.debug(f"Loaded image from {image_path}: {image.width}x{image.height}")
            return image
    ...
    def resize_image(image: Image.Image, max_size: Optional[int] = None) -> Image.Image:
        ...
        logger.debug(f"Resized image: {image.width}x{image.height} -> {resized.width}x{resized.height}")
        return resized
